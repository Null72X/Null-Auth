...
export default function HwidPage() {
  ...
  const handleUpdateHwidString = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!itemToChangeHwid || !newHwidString.trim()) return;
    await fetchApi(`/admin/hwid/${itemToChangeHwid.id}`, {
      method: 'PATCH',
      body: JSON.stringify({ hwid: newHwidString.trim() }),
    });
  ...
  return (
    <div className="space-y-8 animate-fade-in">
              ...
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-xs text-zinc-500 animate-pulse">
                    Loading HWID access list...
                  </td>
                </tr>
              ) : hwids.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-xs text-zinc-500">
                    No authorized HWIDs found matching criteria.
                  </td>
                </tr>
              ) : (
                hwids.map((item) => (
                  <tr key={item.id} className="hover:bg-zinc-800/40 transition-colors">
                    <td className="p-4 font-mono font-bold text-zinc-200">
                      <div className="flex items-center gap-2">
                        <span className="bg-zinc-950 px-2.5 py-1 rounded-lg border border-zinc-800 text-xs truncate max-w-[280px]">
                          {item.hwidHash}
                        </span>
                        <button
                    ...
                    <td className="p-4 font-mono text-xs">
                      <div className="space-y-0.5">
                        <div className="font-bold text-zinc-200">
                          {item.remainingDays > 0 ? `${item.remainingDays} days left` : 'Expired'}
                        </div>
      ...
      {/* HWID Changer Modal */}
      <Modal
        isOpen={!!itemToChangeHwid}
        onClose={() => setItemToChangeHwid(null)}
        title="HWID Changer — Update Authorized Machine SID"
      >
        <form onSubmit={handleUpdateHwidString} className="space-y-4">
          <p className="text-xs text-zinc-400">
            Enter the new Machine SID / HWID string for this user. The application authorization will immediately transfer to the new HWID.
          </p>

          <div>
            <label className="block text-xs font-semibold text-zinc-300 uppercase tracking-wider mb-2">
              New Machine SID / HWID String *
            </label>
            <input
              type="text"
              value={newHwidString}
              onChange={(e) => setNewHwidString(e.target.value)}
              required
              placeholder="e.g. S-1-5-21-3242978600-3580298177-1240389739-1001"
              className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3.5 py-2.5 text-xs font-mono text-purple-400 focus:outline-none focus:border-purple-500"
            />
          </div>

          <div className="flex justify-end gap-3 pt-4 border-t border-zinc-800">
            <Button type="button" variant="secondary" onClick={() => setItemToChangeHwid(null)}>
              Cancel
            </Button>
            <Button type="submit">Save & Change HWID</Button>
          </div>
        </form>
      </Modal>
